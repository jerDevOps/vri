                        ...
                        {MOCK_DATA.events.map((evt) => (
                            <div key={evt.id} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col sm:flex-row items-center gap-8 group reveal">
                                ...
                                <div className="flex-1 text-center sm:text-left">
                                    <span className="inline-block px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-bold uppercase rounded-full mb-2">{evt.type}</span>
                                    <h3 className="text-xl font-bold text-gray-800 mb-2 group-hover:text-unap-blue transition-colors">{evt.title}</h3>
                                    <div className="flex flex-col sm:flex-row gap-4 text-sm text-gray-500">
                                        <span className="flex items-center justify-center sm:justify-start"><i className="far fa-clock mr-2 text-unap-gold"></i> {evt.time}</span>
                                        <span className="flex items-center justify-center sm:justify-start"><i className="fas fa-map-pin mr-2 text-unap-gold"></i> {evt.location}</span>
                                    </div>
                                </div>

                                <button className="flex-shrink-0 w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center text-gray-400 group-hover:bg-unap-blue group-hover:border-unap-blue group-hover:text-white transition-all">
