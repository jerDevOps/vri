...
const Convocatorias = () => {
    ...
    const proximas = convocatoriasData.filter(c => c.state === 'Activa');
    const cerradas = convocatoriasData.filter(c => c.state === 'Cerrado');
    const dataDisplay = selectedTab === 'activas' ? proximas : cerradas;

    const categories = ['todos', ...new Set(convocatoriasData.map(c => c.category.toLowerCase()))];
                        ...
                        .map((item) => (
                            <div key={item.id} className="group bg-white rounded-[2rem] p-8 border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col relative overflow-hidden">
                                ...
                                <div className="flex justify-between items-start mb-6 relative z-10">
                                    <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${item.state === 'Activa' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                                        {item.state}
                                    </span>
                                ...
                                <div className="pt-6 relative z-10 flex gap-4">
                                    ...
                                    >
                                        Ver Bases <i className="fas fa-file-pdf ml-2"></i>
                                    </a>
                                    {item.state === 'Activa' && (
                                        <a
                                            href={item.url}
                                        ...
                                        >
                                            Postular <i className="fas fa-external-link-alt ml-2"></i>
                                        </a>
                                    )}
                                </div>
                            </div>
